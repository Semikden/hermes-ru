...
|-----------|-------|------------|
| Progress-сообщения (инструменты) | ~35 | ✅ 100% |
| Системные сообщения шлюза | ~15 | ✅ 100% |
| Описания команд (/queue, /steer, etc) | ~60 | ✅ 100% |
| Voice-статусы (/voice on/off/tts/status) | ~10 | ✅ 100% |
| Status-сообщения (очередь, итерации) | ~15 | ✅ 100% |
| Подтверждения/отклонения команд | ~5 | ✅ 100% |
| Диагностика (timeout, stuck agent) | ~10 | ✅ 100% |
| Управление рассуждениями (reasoning) | ~5 | ✅ 100% |
| Приоритетная обработка (fast/priority) | ~5 | ✅ 100% |
| Сессии (resume, title, branch) | ~10 | ✅ 100% |

**Итого:** ~180 строк перевода, 2 файла (`run.py` + `commands.py`), 100%
