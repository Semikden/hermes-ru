...
### 4. Voice-статусы (/voice on/off/tts/status)
```
Голосовой режим включён.
Буду отвечать голосом на голосовые сообщения.
Авто-TTS включён. Все ответы будут содержать голосовое сообщение.
Голосовой режим: Выкл (только текст)
Голосовой канал: #general | Участников: 3
```
Файл: `gateway/run.py` — `_handle_voice_command()`
